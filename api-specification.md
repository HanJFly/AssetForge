...
说明：是否支持额外筛选字段，以各模块当前后端 `Param` / `Controller` 为准；`keyword` 已废弃，不再作为通用分页参数。
...
说明：当前后端 `ReturnOrderController` 仅已实现 `POST /return-order/create`。本节其余接口为待后端补齐的预留设计，联调时以后端新增方法为准。
...
说明：当前后端仅存在 `@RequestMapping("/scrapOrder")` controller 根路径，具体 `create/page/detail` 方法尚未实现。本节路径命名已按后端根路径保留。

### 14.1 创建报废单

`POST /scrapOrder/create`
...
说明：当前后端仅存在 `@RequestMapping("/inventoryTask")` 与 `@RequestMapping("/inventoryDetail")` controller 根路径，具体方法尚未实现。本节路径命名已按后端根路径保留。

### 15.1 创建盘点任务

`POST /inventoryTask/create`
...
说明：当前后端仅存在 `@RequestMapping("/lossOrder")` controller 根路径，具体 `page/detail/handle` 方法尚未实现。本节路径命名已按后端根路径保留。
...
`POST /depreciationRunLog/execute`

说明：当前后端仅存在 `@RequestMapping("/depreciationRunLog")` controller 根路径，`execute` / `page` 方法尚未在 controller 中实现；本节先按后端根路径记录。
...
说明：当前后端 `FileAttachmentController` 尚未提供列表方法，本节为预留设计；当前已实现的是 `upload` 与 `bind`。
